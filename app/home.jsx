...
function TodayPreparation({ go, progress, review, onStartReview }) {
  ...
  return (
    <section className="today-prep">
      <div className="today-prep-head"><div><span className="eyebrow small"><span className="eyebrow-line" /> Your next session</span><h3>Today’s preparation</h3><p>A small loop: recall what is due, attempt today’s questions, then reinforce your weakest area.</p></div><div className="today-prep-time"><strong>~{minutes} min</strong><span>{dailyDone && !dueQuestions ? "Momentum maintained" : "Ready when you are"}</span></div></div>
      <div className="today-prep-steps">{steps.map((step, index) => <button key={step.key} className={`today-prep-step${step.done ? " done" : ""}`} onClick={step.action}><span className="today-prep-number">{step.done ? "✓" : String(index + 1).padStart(2, "0")}</span><span><em>{step.label}</em><strong>{step.title}</strong><small>{step.meta}</small></span><Icon name="arrowR" size={15} /></button>)}</div>
